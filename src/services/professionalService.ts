...
const profileKey = (id: string) =>
    `talentsphere_professional_profile_${id}`;

const evidenceKey = (id: string) =>
    `talentsphere_professional_evidence_${id}`;

export const emptyProfessionalProfile = (
    name = '',
    email = ''
): ProfessionalProfile => ({
    fullName: name,
...
export function getProfessionalEvidence(
    id: string
): ProfessionalEvidence {
    try {
        return {
...
export function saveProfessionalEvidence(
    id: string,
    v: ProfessionalEvidence
) {
    localStorage.setItem(
        evidenceKey(id),
        JSON.stringify(v)
...
export function professionalStats(
    ...
    const p = getProfessionalProfile(
        id,
        name,
        email
    );
    ...
    const resume = Math.min(
        ...
        (p.company ? 15 : 0) +
        (p.linkedin ? 15 : 0) +
        (p.portfolio || p.github
            ? 15
            : 0) +
        (e.resumeText.trim().length >
            120
            ? 20
            : 0)
    );

    const certification = Math.min(
        100,
        e.certifications.length * 20
    );

    const learning = Math.min(
        100,
        e.learningCompleted.length * 20
    );
