...
function AcademicProgress() {
    ...
    useEffect(() => {
        ...
        setSubjects(getSubjectProgress(user.id));
    }, [user]);

    if (!user) return null;

    const stats = calculateWorkspaceStats(user.id);

    const analyses = getUserAssessmentResults(user.id);
    ...
    const overall = Math.round(
        (stats.overall + assessmentAverage) / 2
    );

    // ✅ Fixed Function
    const save = (name: string, value: number) => {
    ...
    const cards = [
        ...
        [
            ...
            String(stats.completedAssignments),
            Award,
        ],
    ] as const;
