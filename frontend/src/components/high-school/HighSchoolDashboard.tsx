...
function HighSchoolDashboard() {
  ...
  useEffect(() => {
    ...
    const savedProfile = getHighSchoolProfile(user.id);
  ...
  const profileCompletion = dashboard?.profile.profile_completion ?? calculateProfileCompletion(profile);

  if (!user) {
    return null;
  }

  const currentStream = dashboard?.profile.stream ?? (profile
    ? (profile.studentClass === "9" || profile.studentClass === "10")
      ? "Not Selected Yet"
      : profile.currentStream ?? "Not Set"
    : "Not Set");

  const assessment = dashboard?.assessment.high_school;
  const latestScore = assessment?.completed ? assessment.percentage : null;
    ...
    <div className="mx-auto max-w-[1440px] space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <h3 className="mt-2 text-xl font-bold text-slate-900">
              {(dashboard?.profile.student_class || profile?.studentClass || "Not Set").replace(/^Class\s+/i, "Class ")}
            </h3>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <h3 className="mt-2 text-xl font-bold text-slate-900">
              {dashboard?.profile.board ?? profile?.board ?? "Not Set"}
            </h3>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <h3 className="mt-2 text-lg font-bold text-slate-900">
              {dashboard?.profile.target_role || dashboard?.profile.student_class && profile?.careerGoal || profile?.careerGoal || "Exploring"}
            </h3>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <h3 className="mt-2 text-xl font-bold text-slate-900">
            {latestScore !== null ? `${latestScore}%` : "Not completed"}
          </h3>
          <p className="mt-1 text-sm text-slate-500">
            {dashboard?.assessment.total_completed ?? 0} completed assessment(s)
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Career Direction</p>
          <h3 className="mt-2 text-xl font-bold text-slate-900">
            {dashboard?.career.recommended_role ?? "Exploring"}
          </h3>
          <p className="mt-1 text-sm text-slate-500">
            {dashboard?.career.confidence != null ? `${dashboard.career.confidence}% confidence` : "Complete assessment for guidance"}
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Latest Activity</p>
          <h3 className="mt-2 text-lg font-bold text-slate-900">
            {dashboard?.assessment.latest_assessment?.replaceAll("_", " ") ?? "No assessment yet"}
          </h3>
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Your profile information and assessment progress.
                </p>
              </div>
            </div>
        ...
        <div className="rounded-2xl border border-cyan-100 bg-cyan-50/60 p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Choose any assessment that matches your current interest. Your profile stream does not lock your choices.
          </p>
