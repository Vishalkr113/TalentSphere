    ...
    const review = async () => {
        ...
        try {
            const result = await generateAI(
                'mock_interview',
                `Interview question: ${qs[i]}\n\nCandidate answer: ${text}`,
                `Target role: ${p.targetRole}. Resume projects: ${r.projects || 'not supplied'}. Priority skill gap: ${gap?.skill || 'none identified'}.`,
            );
            setFeedback(result.text);
        } catch (error) {
            setFeedback(error instanceof Error ? error.message : 'AI feedback is temporarily unavailable.');
        } finally {
            setLoadingAI(false);
        }
    };
