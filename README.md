...
Frontend: copy `frontend/.env.example` to `frontend/.env` and set `VITE_API_URL` to the backend URL.

Backend: copy `backend/.env.example` to `backend/.env`, set a strong random `SECRET_KEY`, configure the database/email/AI settings you need, and set `CORS_ORIGINS` to the exact frontend origin(s).

Never commit or distribute `.env` files or real credentials.
